...
| `DB_POOL` | `5` | Max pooled connections per machine (1–50) — keep small when several PTA machines + TapIn School share one MySQL (`max_connections` is usually 151). Raise it only on a very busy admin PC |
